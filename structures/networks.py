...
#
#         self.l1 = nn.Linear(states_dim, 256)
...
class Actor(torch.nn.Module):
    def __init__(self,
        ...
        self.l5 = nn.Linear(hidden_units, action_dim)
        self.max_action = max_action

    def forward(self, state: torch.Tensor) -> torch.Tensor:
        l2_input = self.l1(state).relu()
        l3_input = self.l2(l2_input).relu()
        l4_input = self.l3(l3_input).relu()
        l5_input = self.l4(l4_input).relu()
        y = torch.tanh(self.l5(l5_input))*self.max_action
...
class Critic(torch.nn.Module):
    def __init__(self,
        ...
        self.l3 = nn.Linear(hidden_units, hidden_units)
        self.l4 = nn.Linear(hidden_units, hidden_units)
        self.l5 = nn.Linear(hidden_units, 1)
