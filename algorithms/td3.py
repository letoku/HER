...
from algorithms.abstract_classes import Agent
from structures.networks import Actor, Critic
from structures.replay_memory import ReplayMemory
from structures.standardizer import Standardizer
...
class TD3Agent(Agent):
    def __init__(self, memory_size: int, states_dim: int, actions_dim: int, action_min: float,
                 ...
                 policy_update_delay: int = 2, her_clip: bool = True):
        ...
        self.states_standardizer = Standardizer(input_dim=states_dim, device=device, lower_bound=-5, upper_bound=5)
        self.actions_standardizer = Standardizer(input_dim=actions_dim, device=device, lower_bound=-5, upper_bound=5)
        ...
        self.policy_optimizer = torch.optim.Adam(self.policy.parameters(), lr=self.pi_learning_rate, weight_decay=0.0001)
    ...
    def get_action(self, state: np.ndarray, noise: float = None):
        ...
        if self.standardizing:
            policy_input = self.states_standardizer.standarize(policy_input)
        ...
        return np.squeeze(np.clip(action, a_min=self.action_min, a_max=self.action_max), axis=0)
    ...
    def update(self, batch_size: int = None):
        ...
        if self.standardizing:
            batch['states'] = self.states_standardizer.standarize(batch['states'])
            batch['next_states'] = self.states_standardizer.standarize(batch['next_states'])
            batch['actions'] = self.actions_standardizer.standarize(batch['actions'])
    ...
    def _calc_targets(self, batch):
        with torch.no_grad():
            ...
            target = batch['rewards'] + (1 - batch['terminated']) * self.gamma*torch.minimum(y1, y2)
            if self.her_clip:
                return torch.clip(target, min=-1/(1-self.gamma), max=0)
    ...
    def push(self, transition):
        if self.standardizing:
            self.states_standardizer.add_data(transition.state)
            self.actions_standardizer.add_data(transition.action)
